...
function vpadding(element) {
	var float1 = parseFloat($(element[0]).css('paddingTop'))
	var float2 = parseFloat($(element[0]).css('paddingBottom'))
	return (float1 || 0) + (float2 || 0);
}
...
function View(element, calendar, viewName) {
	...
	function isEventDraggable(event) {
		return false; //return isEventEditable(event) && !opt('disableDragging');
	}
	
	
	function isEventResizable(event) { // but also need to make sure the seg.isEnd == true
		return false; //return isEventEditable(event) && !opt('disableResizing');
	}
	
	
	function isEventEditable(event) {
		return false; //return firstDefined(event.editable, (event.source || {}).editable, opt('editable'));
	}
	...
	function eachEventElement(event, exceptElement, funcName) {
		...
		for (i=0; i<len; i++) {
			if (!exceptElement || elements[i][0] != exceptElement[0]) {
				//console.log(elements[i][funcName])
				elements[i][funcName]();
			}
		}
	}
