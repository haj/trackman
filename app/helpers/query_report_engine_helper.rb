...
module QueryReportEngineHelper
  ...
  def query_report_search_form(report, &block)
    if report.array_record?
      form_tag(url_for({}), ({class: 'form-inline'} || {}).merge({method: :get, remote: @remote}), &block)
    else
      search_form_for(report.search, url: url_for({}), remote: @remote, html: ({class: 'form-inline'} || {}).merge(method: :get), &block)
    end
  end
end