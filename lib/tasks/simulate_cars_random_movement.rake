namespace :simulate do
  task :cars_random_movement => :environment do
  	Traccar::Device.all.each do |device|
        ...
        if random_choice == 0 
          ...
          position = Traccar::Position.create(altitude: 0.0, course: 0.0, latitude: device.last_position[:latitude], longitude: device.last_position[:longitude], other: "<info><protocol>t55</protocol><battery>24</battery...", power: nil, speed: 0.0, time: Time.now, valid: true, device_id: device.id)
          device.positions << new_position
        end		
